...
import sys
sys.path.append("./yolov5_ultralytics")
...
def get_featuresdict(model, dir):
    person_dict = {}
    folder_list = os.listdir(dir)  # 获取pic目录下的文件夹列表
    for folder in folder_list:
        folder_path = os.path.join(dir, folder)  # 获取每个文件夹的完整路径
        if os.path.isdir(folder_path):  # 确保是文件夹
            image_list = os.listdir(folder_path)  # 获取文件夹中的文件列表
            for image_name in image_list:
                image_path = os.path.join(folder_path, image_name)  # 获取每个图像的完整路径
                image = load_image(image_path)
                if image is not None:  # 确保图像加载成功
                    ...
                    person_dict[image_name] = feature
    return person_dict


def detect(opt,save_img=False):
    out, source, yoloV5_weights, Arcface_weights, view_img, save_txt, imgsz = \
        opt.output, opt.source, opt.yoloV5_weights, opt.Arcface_weights, opt.view_img, opt.save_txt, opt.img_size  # 加载配置信息
    ...
    arcface_model.load_state_dict(torch.load(Arcface_weights), strict=False)
                    ...
                    for folder in folder_list:
                        ...
                        if os.path.isdir(folder_path):  # 确保是文件夹
                            ...
                            for each in image_list:
                                ...
                                if max_f > 0.44:
                                    # 获取上一级目录名
                                    label = "_".join(max_n.split("_")[:-1])
                                    # label = max_n[:-4]
                                    # filename_without_extension = os.path.splitext(max_n)[0]  # 去除扩展名
                                    # label = "_".join(filename_without_extension.split("_")[:-1])
...
def detect_arcface(choice):
    parser = argparse.ArgumentParser()
    parser.add_argument('--yoloV5_weights', type=str, default=r'weights/face.pt', help='yolov5.pt path')
    parser.add_argument('--Arcface_weights', type=str, default=r'weights/resnet18_110.pth', help='arcface.pth path')
