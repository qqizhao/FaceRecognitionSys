...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    # Check anchor fit to icons, recompute if necessary
...
def coco80_to_coco91_class():  # converts 80-index (val2014) to 91-index (paper)
    ...
    # a = np.loadtxt('icons/coco.names', dtype='str', delimiter='\n')
    # b = np.loadtxt('icons/coco_paper.names', dtype='str', delimiter='\n')
...
def crop_images_random(path='../input/', scale=0.50):  # from utils.utils import *; crop_images_random()
    # crops input into random squares up to scale fraction
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    ...
    os.makedirs('new/input/')
    for file in tqdm(sorted(glob.glob('%s/*.*' % path))):
        with open(file, 'r') as f:
            labels = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)
        i = labels[:, 0] == label_class
        if any(i):
            img_file = file.replace('labels', 'input').replace('txt', 'jpg')
            labels[:, 0] = 0  # reset class to 0
            with open('new/input.txt', 'a') as f:  # add image to dataset list
                f.write(img_file + '\n')
            with open('new/labels/' + Path(file).name, 'a') as f:  # write label
                for l in labels[i]:
                    f.write('%g %.6f %.6f %.6f %.6f\n' % tuple(l))
            shutil.copyfile(src=img_file, dst='new/input/' + Path(file).name.replace('txt', 'jpg'))  # copy input


def kmean_anchors(path='./icons/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
...
def plot_images(images, targets, paths=None, fname='input.jpg', names=None, max_size=640, max_subplots=16):
    tl = 3  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    ...
    bs = min(bs, max_subplots)  # limit plot input
    ...
    for i, img in enumerate(images):
        if i == max_subplots:  # if last batch has fewer input than we expect
            break
...
def plot_study_txt(f='study.txt', x=None):  # from utils.utils import *; plot_study_txt()
    # Plot study.txt generated by test6.py
